...
import { UINode, ParseResult, ParseError, parseUI, serializeUI } from '../../parser';
...
function defaultPropsForTag(tag: string): Record<string, string> {
...
function removeNodeFromTree(nodes: UINode[], nodeId: string): UINode[] {
  let changed = false;
  const result: UINode[] = [];
  for (const node of nodes) {
    if (node.id === nodeId) {
...
function isDescendant(node: UINode, targetId: string): boolean {
